...
import type { DisplayToggle, MultiItemActivityForm } from '../types'

const CODE_MAX = 30

interface MultiItemAdvancedOptionsFieldsProps {
  activity: MultiItemActivityForm
  readOnly?: boolean
  onChange: (activity: MultiItemActivityForm) => void
}

export function MultiItemAdvancedOptionsFields({ activity, readOnly, onChange }: MultiItemAdvancedOptionsFieldsProps) {
    ...
    <div className="form-section form-section--collapsible">
      ...
      {expanded && (
        <div className="form-section__body">
          ...
          <FormRow label="参与用户">
            <div>
              <select className="input input--wide" disabled={readOnly} value={activity.participantUser} onChange={(e) => update('participantUser', e.target.value as MultiItemActivityForm['participantUser'])}>
...
function DisplayRadio({ value, onChange, disabled }: { value: DisplayToggle; onChange: (v: DisplayToggle) => void; disabled?: boolean }) {
  return (
    <div className="radio-group">
      <label className="radio"><input type="radio" disabled={disabled} checked={value === 'show'} onChange={() => onChange('show')} />展示</label>
      <label className="radio"><input type="radio" disabled={disabled} checked={value === 'hide'} onChange={() => onChange('hide')} />不展示</label>
    </div>
  )
}
...
export function formatMultiItemAdvancedPreview(activity: MultiItemActivityForm) {
